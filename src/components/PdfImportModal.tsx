...
const PdfImportModal = ({ open, onClose }: Props) => {
  ...
  const toggleResult = (key: string) => {
    setResults(prev => prev.map(r => r.key === key ? { ...r, selected: !r.selected } : r));
  };
  ...
  const handleSave = () => {
    ...
    toast.success(`تم حفظ ${labResults.length} نتيجة تحليل بنجاح ✓`);
    handleClose();
  };
    ...
    <div className="fixed inset-0 bg-foreground/40 z-50 flex items-end sm:items-center justify-center">
      <div className="bg-card w-full max-w-lg mx-auto rounded-t-3xl sm:rounded-3xl p-5 max-h-[90vh] flex flex-col animate-slide-up">
        ...
        <div className="flex items-center justify-between mb-4 flex-shrink-0">
          <h2 className="text-lg font-bold">📎 استيراد نتائج من PDF</h2>
        ...
        {step === 'upload' && (
          <div className="flex-1 flex flex-col items-center justify-center py-10">
            ...
            <p className="text-lg font-semibold mb-2">اختر ملف PDF</p>
            <p className="text-sm text-muted-foreground mb-6 text-center">قم بتحميل تقرير التحاليل المخبرية بصيغة PDF وسيتم استخراج النتائج تلقائياً</p>
            <input ref={fileRef} type="file" accept=".pdf" className="hidden"
              onChange={e => { const f = e.target.files?.[0]; if (f) handleFile(f); }} />
            <button onClick={() => fileRef.current?.click()}
              className="gradient-primary text-primary-foreground font-bold py-3 px-8 rounded-2xl text-lg">
              اختر ملف PDF
            </button>
          </div>
        )}
        ...
        {step === 'loading' && (
          <div className="flex-1 flex flex-col items-center justify-center py-10">
            ...
            <p className="text-lg font-semibold">جاري تحليل التقرير...</p>
            <p className="text-sm text-muted-foreground mt-2">يتم استخراج النتائج من ملف PDF</p>
          </div>
        )}
        ...
        {step === 'error' && (
          <div className="flex-1 flex flex-col items-center justify-center py-10">
            ...
            <p className="text-lg font-semibold mb-2">لم نتمكن من قراءة التقرير</p>
            <p className="text-sm text-muted-foreground mb-6 text-center">قد يكون الملف غير مدعوم أو لا يحتوي على نتائج تحاليل معروفة</p>
            <div className="flex gap-3">
              <button onClick={() => { setStep('upload'); }}
                className="bg-primary text-primary-foreground font-bold py-3 px-6 rounded-2xl">
                حاول مرة أخرى
              </button>
              <button onClick={handleClose}
                className="bg-secondary font-bold py-3 px-6 rounded-2xl">
                إدخال يدوي
              </button>
            </div>
          </div>
        ...
        {step === 'preview' && extraction && (
          <>
            ...
            <div className="bg-secondary/50 rounded-xl p-3 mb-3 flex-shrink-0">
              <h3 className="font-bold text-sm mb-1">النتائج المستخرجة من التقرير</h3>
              <div className="flex flex-wrap gap-2 text-xs text-muted-foreground">
                {extraction.patientName && <span>👤 {extraction.patientName}</span>}
                {extraction.gender && <span>• {extraction.gender === 'male' ? 'ذكر' : 'أنثى'}</span>}
                {extraction.age && <span>• {extraction.age} سنة</span>}
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between">
                        ...
                        <span className={`text-xs px-2 py-0.5 rounded-full font-semibold ${
                          r.status === 'normal' ? 'status-normal' : r.status === 'warning' ? 'status-warning' : 'status-danger'
                        }`}>
            ...
            <button onClick={handleSave} disabled={selectedCount === 0}
              ...
              حفظ النتائج المحددة ✓ ({selectedCount})
            </button>
          </>
        )}
      </div>
